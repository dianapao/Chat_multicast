...
public class Ventana implements ActionListener{
    ...
    private ArrayList<JButton> botonesFile;
    ...
    public Ventana(int operacion){
        statusOp = operacion;
        salida = 0;
        Nombre = JOptionPane.showInputDialog("Ingresa el nombre de usuario");
    ...
    private void inicializarComponentes(){
        panel = new JPanel();
        JLabel titulo = new JLabel("USUARIO: " + Nombre);
    ...
    private void setButtons(){
        ...
        newChat.setBounds(200, 10, 180, 30);
        panel.add(newChat);
    }
    ...
    private void newChat(String nombre){
        ...
        textoEnviar.get(textoEnviar.size() - 1).setBounds(10, 320, 310, 39);
        newPanel.add(textoEnviar.get(textoEnviar.size() - 1));
        
        JButton enviar = new JButton("Enviar");
        botonesEnviar.add(enviar);
        botonesEnviar.get(botonesEnviar.size() - 1).setBounds(400, 320, 70, 38);
        ...
        System.out.println("butname:" + butname);
        ...
        botonesFile.add(fileSend);
        botonesFile.get(botonesFile.size()-1).setBounds(330,320,65,38);
        botonesFile.get(botonesFile.size()-1).setText("File");
        botonesFile.get(botonesFile.size()-1).addActionListener(this);
        String butFileName = "File" + (botonesFile.size() - 1);
        botonesFile.get(botonesFile.size()-1).setName(butFileName);
        newPanel.add(botonesFile.get(botonesFile.size() -1 ));
    ...
    public ArrayList<String> getContactosChat(){
        return contactos;
    }
    ...
    public void setNewMessage(String mensaje){
        if(mensaje.contains("<contactos>")){
            ...
            textoEnviar.get(0).setText("");
            chatPersona.get(0).setText(chatPersona.get(0).getText() + "\n" + contacto + " ha iniciado sesión");
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(boton.getActionCommand().equals("Nuevo chat privado")){
            ...
            statusOp = 1;
        }else if(boton.getActionCommand().equals("File")){                    
